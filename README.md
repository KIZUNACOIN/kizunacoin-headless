# Headless wallet for Kizunacoin network

This is a headless alternative of the [GUI wallet](../../../kizunacoin) for Kizunacoin network.  It is designed for an always-online deployment on a server.
...
If you want to change any defaults, refer to the documentation of [core](../../../core), the core Kizunacoin library `require()`'d from here.  Below are some headless wallet specific settings you might want to change:
...
* `payout_address`: if you give `pay` command over chat interface, the money will be sent to this Kizunacoin address.
* `hub`: hub address without wss://, the default is `hub.kizunacoin.jp/`.
...
Payments are the central but not the only type of data that Kizunacoin stores.  In [play/](play/) subdirectory, you will find many small scripts that demonstrate how to create other message types that are not available through the GUI wallet.  In particular, you can declare and issue your own assets, post data as an oracle, create polls and cast votes.  Just edit any of these scripts and run it.
